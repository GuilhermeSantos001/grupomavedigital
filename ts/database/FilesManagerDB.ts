...
import { FoldersSchema } from '@/schemas/FoldersSchema';
import { PrivilegesSystem } from "@/schemas/UsersSchema";
import { MetadataFile } from '@/drivers/FileGridFS';
import { FoldersManagerDB, Access as AccessFolder } from '@/database/FoldersManagerDB';
...
export class FilesManagerDB {
    ...
    public async recycleGarbage(): Promise<string[]> {
        const
            files = await FilesSchema.find({ trash: { $ne: undefined } }),
            itens: string[] = [];
            ...
            for (const file of files) {
                if (file.trash instanceof Date) {
                    ...
                    if (now > file.trash && file.status !== 'Recycle') {
                        ...
                        await FilesSchema.updateOne({ cid: file.cid }, {
                            $set: {
                                status: file.status,
    ...
    public async verifyBlocked(cid: string, access: Access): Promise<BlockVerify> {
        const file = await FilesSchema.findOne({ cid });
    ...
    public async get(filter: FilterQuery<fileModelInterface>, skip: number, limit: number): Promise<fileModelInterface[]> {
        ...
        const _files = await FilesSchema.find(filter, null, { skip, limit }).exec();
    ...
    public async save(file: fileInterface): Promise<fileModelInterface> {
        ...
        const _file = await FilesSchema.findOne({ name: file.name, type: file.type });
        ...
        const
            now = Moment.format(),
            model = await FilesSchema.create({
                ...file,
                cid: uuid,
                updated: now,
                lastAccess: now,
    ...
    public async update(cid: string, access: Access, data: Pick<fileInterface, "name" | "description" | "tag">): Promise<boolean> {
         ...
         */
        const _exist = await FilesSchema.findOne({ name: data.name, type: file.type });
    ...
    public async delete(cid: string, access: Access, accessFolder: AccessFolder): Promise<boolean> {
        ...
        if (_delete) {
            ...
            if (this._isAssociatedFolder(file)) {
                try {
                    const foldersManagerDB = new FoldersManagerDB();

                    await foldersManagerDB.remove(file.folderId || "", accessFolder, cid, access);
                    await foldersManagerDB.close(file.folderId || "", accessFolder);
                } catch (error) {
                    throw new Error(`A pasta(${file.folderId}) está protegida/bloqueada ou na lixeira, não é possível remover o arquivo no momento.`);
                }
            }

            await FilesSchema.findOneAndRemove({ cid: file.cid });
    ...
    public async joinFolder(cid: string, access: Access, folderId: string): Promise<boolean> {
        const
            file = await FilesSchema.findOne({ cid }),
            folder = await FoldersSchema.findOne({ cid: folderId });
    ...
    public async rename(cid: string, access: Access, name: string): Promise<{ filesID: ObjectId[], type: string } | boolean> {
         ...
         * ? Verifica se existe outro arquivo com o novo nome
         */
        const _exist = await FilesSchema.findOne({ name, type: file.type });
