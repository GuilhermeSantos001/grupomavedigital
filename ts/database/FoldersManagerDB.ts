...
} from '@/schemas/FoldersSchema';
import { FilesSchema, FileShare, FileProtected, FileBlocked } from '@/schemas/FilesSchema';
...
export class FoldersManagerDB {
    ...
    public async recycleGarbage(): Promise<string[]> {
        const
            folders = await FoldersSchema.find({ trash: { $ne: undefined } }),
            itens: string[] = [];
            ...
            for (const folder of folders) {
                if (folder.trash instanceof Date) {
                    ...
                    if (now > folder.trash && folder.status !== 'Recycle') {
                        ...
                        await FoldersSchema.updateOne({ cid: folder.cid }, {
                            $set: {
    ...
    public async verifyBlocked(cid: string, access: Access): Promise<BlockVerify> {
        const
            folder = await FoldersSchema.findOne({ cid });
    ...
    public async get(filter: FilterQuery<folderModelInterface>, skip: number, limit: number): Promise<folderModelInterface[]> {
        ...
        const _folders = await FoldersSchema.find(filter, null, { skip, limit }).exec();
    ...
    public async name(cid: string): Promise<string> {
        const folder = await FoldersSchema.findOne({ cid });
    ...
    public async save(folder: folderInterface): Promise<folderModelInterface> {
        ...
        const _folder = await FoldersSchema.findOne({ name: folder.name, type: folder.type });
        ...
        const
            now = Moment.format(),
            model = await FoldersSchema.create({
                ...folder,
                cid: uuid,
                updated: now,
                lastAccess: now,
        ...
        if (_delete) {
            ...
            if (folder.filesId instanceof Array && folder.filesId.length > 0) {
                ...
                for (const fileId of folder.filesId) {
                    try {
                        const filesManagerDB = new FilesManagerDB();

                        await filesManagerDB.exitFolder(fileId, accessFile, cid);
                        await filesManagerDB.close(fileId, accessFile);
                    } catch (error) { errors++; }
                }
            ...
            await FoldersSchema.findOneAndRemove({ cid: folder.cid });
    ...
    public async append(cid: string, access: Access, fileId: string, accessFile: AccessFile): Promise<boolean> {
        const
            folder = await FoldersSchema.findOne({ cid }),
            file = await FilesSchema.findOne({ cid: fileId });
        ...
        if (_append) {
            ...
            try {
                ...
                if (file.folderId) {
                    const parent = await FoldersSchema.findOne({ cid: file.folderId });
                    ...
                    if (parent) {
                        ...
                        await filesManagerDB.exitFolder(fileId, accessFile, file.folderId);
                        await filesManagerDB.close(fileId, accessFile);

                        parent.updated = Moment.format();

                        await FoldersSchema.updateOne({ cid: parent.cid }, {
                            $set: {
                ...
                await filesManagerDB.joinFolder(fileId, accessFile, cid);
                await filesManagerDB.close(fileId, accessFile);
            } catch (error) {
    ...
    public async remove(cid: string, access: Access, fileId: string, accessFile: AccessFile): Promise<boolean> {
        ...
        if (_remove) {
            ...
            try {
                ...
                if (folder.folderId) {
                    const parent = await FoldersSchema.findOne({ cid: folder.folderId });
                    ...
                    if (parent) {
                        ...
                        await filesManagerDB.joinFolder(fileId, accessFile, parent.cid || "");
                        await filesManagerDB.close(fileId, accessFile);
    ...
    public async push(cid: string, access: Access, folderId: string): Promise<boolean> {
        const
            folder = await FoldersSchema.findOne({ cid }),
            folder2 = await FoldersSchema.findOne({ cid: folderId });
        ...
        if (_push) {
            ...
            await FoldersSchema.updateOne({ cid: folder2.cid }, {
                $set: {
    ...
    public async moveToGarbage(cid: string, access: Access, accessFile: AccessFile): Promise<boolean> {
        ...
        if (this._isAvailable(folder)) {
            const
                filesManagerDB = new FilesManagerDB(),
                now = new Date();
            ...
            if (folder.filesId instanceof Array && folder.filesId.length > 0) {
                ...
                for (const fileId of folder.filesId) {
                    try {
                        await filesManagerDB.moveToGarbage(fileId, accessFile);
                    } catch (error) { errors++; }
                }
    ...
    public async removeOfGarbage(cid: string, access: Access, accessFile: AccessFile): Promise<boolean> {
        ...
        if (this._isGarbage(folder)) {
            ...
            if (folder.filesId instanceof Array && folder.filesId.length > 0) {
                ...
                for (const fileId of folder.filesId) {
                    try {
                        await filesManagerDB.removeOfGarbage(fileId, accessFile);
                    } catch (error) { errors++; }
                }
