...
describe("Teste da database de uploads", () => {
  ...
  it('Registra um upload', async () => {
    const register = await uploadDB.register({
      authorId: 'tester',
      fileId: '123456789',
      filename: 'teste',
      filetype: '.txt',
  ...
  it('Remove um upload', async () => {
    ...
    const remove = await uploadDB.remove('12345678910');

    expect(remove).toBe(true);
  })

  it('Torna um upload permanente', async () => {
    const makePermanent = await uploadDB.makePermanent('123456789');

    expect(makePermanent).toBe(true);
  })

  it('Torna um upload temporario', async () => {
    const makeTemporary = await uploadDB.makeTemporary('123456789', new Date(Date.now() + (1000 * 60 * 5)).toISOString());

    expect(makeTemporary).toBe(true);
  })

  it('Retorna todos os uploads', async () => {
    const getAll = await uploadDB.getAll();

    expect(getAll.length).toBe(1);

    expect(getAll[0]).toHaveProperty('authorId');
    expect(getAll[0]).toHaveProperty('fileId');
    expect(getAll[0]).toHaveProperty('name');
