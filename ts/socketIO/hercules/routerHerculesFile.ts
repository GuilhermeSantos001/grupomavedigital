...
import { GroupId, UserId, matches } from "@/mongo/files-manager-mongo";
...
export default function routerHerculesFile(io: Server, socket: Socket): void {
  ...
  socket.on('CREATE-FILE', async (
    ...
    try {
      ...
      if (files.length <= 0) {
        const file = await Files.newFile({
          room,
          authorId,
          accessGroupId: (() => {
            const data: GroupId[] = [];
        ...
        if (
          ...
          await Files.insertVersion(file.cid || "", authorId, size, compressedSize, new ObjectId(fileId), version, { group: { name: 'administrador', permission: 'Write' } });

          const updateFile = await Files.get({
            cid: file.cid
          }, 0, 1);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.remove(file.cid || "", { group: { name: 'administrador', permission: 'Delete' } }, versions);

          const updatedFile = await Files.get({ cid }, 0, 1);
  ...
  socket.on('GET-FILES-BY-ROOM', async (
    ...
    try {
      const files = await Files.get({ room: { $all: room }, folderId: { $eq: !folderId ? undefined : folderId } }, 0, limit),
        { email } = await Users.getInfo(userAuth);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.update(file.cid || "", { group: { name: 'administrador', permission: 'Write' } }, { name: name, description: file.description, tag: file.tag })
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.update(file.cid || "", { group: { name: 'administrador', permission: 'Write' } }, { name: file.name, description, tag: file.tag })
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.update(file.cid || "", { group: { name: 'administrador', permission: 'Write' } }, { name: file.name, description: file.description, tag })
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.insertGroupId(file.cid || "", { group: { name: 'administrador', permission: 'Security' } }, group);
  ...
  socket.on('FILE-ADD-USER', async (
    ...
    try {
      const files = await Files.get({ cid }, 0, 1),
        users = await Users.get({ clearEmail: user.email }, 0, 1);
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.insertUserId(file.cid || "", { group: { name: 'administrador', permission: 'Security' } }, user);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.removeGroupId(file.cid || "", { group: { name: 'administrador', permission: 'Security' } }, group);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.removeUserId(file.cid || "", { group: { name: 'administrador', permission: 'Security' } }, user);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.insertPermissionInGroupId(file.cid || "", { group: { name: 'administrador', permission: 'Security' } }, group, group.permissions);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.insertPermissionInUserId(file.cid || "", { group: { name: 'administrador', permission: 'Security' } }, user, user.permissions);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.removePermissionInGroupId(file.cid || "", { group: { name: 'administrador', permission: 'Security' } }, group, group.permissions);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.removePermissionInUserId(file.cid || "", { group: { name: 'administrador', permission: 'Security' } }, user, user.permissions);
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.moveToGarbage(file.cid || "", { group: { name: 'administrador', permission: 'Security' } });
    ...
    try {
      ...
      if (files.length <= 0) {
        ...
        if (
          ...
          await Files.removeOfGarbage(file.cid || "", { group: { name: 'administrador', permission: 'Security' } });
