...
export class Locale {
  ...
  constructor(private options?: CoreConfig) {
    if (!this.options) this.options = { locale: {} } as CoreConfig;

    if (!this.options.locale.main) this.locale = CoreOptions.locale.main;
    else this.locale = this.options.locale.main;

    if (!this.options.locale.path) this.locales = CoreOptions.locale.languages;
    else this.locales = this.options.locale.languages;

    if (!this.options.locale.path) this.path = CoreOptions.locale.path;
